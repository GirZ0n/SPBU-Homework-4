...
class WcTestCase(unittest.TestCase):
    def test_empty_file(self):
        with open(f"{FOLDER_PATH}/empty.txt", "r") as f:
    ...
    def test_one_line_one_word(self):
        with open(f"{FOLDER_PATH}/one_line_one_word.txt", "r") as f:
            info = get_file_information(f)
    ...
    def test_one_line_a_lot_of_words(self):
        with open(f"{FOLDER_PATH}/one_line_a_lot_of_words.txt", "r") as f:
            info = get_file_information(f)
            self.assertEqual(info.number_of_lines, 1)
    ...
    def test_many_lines(self):
        with open(f"{FOLDER_PATH}/many_lines.txt", "r") as f:
            info = get_file_information(f)
